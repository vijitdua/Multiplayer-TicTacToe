...
export async function joinRoom(req,res,dbConnector){
    try{
        ...
        console.log("A player has joined someone's game room!", {...req.body, roomID: roomID, hostUserName: hostUserName});
        res.json({roomID:roomID, hostUserName: hostUserName, playerUserName: playerUserName, res: "success"});
    }
        // Error catching, and send error data to client
    catch(error){
